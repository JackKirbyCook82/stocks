...
class BarsCalculator(StateCalculator, register=Technical.BARS): pass
class StatsCalculator(StateCalculator, register=Technical.STATS):
...
class SMACalculator(TrendCalculator, register=Technical.SMA):
    sma = lambda adjusted, *, period: adjusted.rolling(window=period).mean()

class EMACalculator(TrendCalculator, register=Technical.EMA):
    ema = lambda adjusted, *, period: adjusted.ewm(span=period, min_periods=period, adjust=False).mean()

class MACDCalculator(TrendCalculator, variables=["macd", "sign", "hist"], register=Technical.MACD):
...
class RSICalculator(MomentumCalculator, variables=["rsi"], register=Technical.RSI):
...
class BBCalculator(VolatilityCalculator, variables=["bbh", "bbl"], register=Technical.BB):
...
class ATRCalculator(VolatilityCalculator, variables=["atr"], register=Technical.ATR):
...
class MFICalculator(VolumeCalculator, variables=["mfi"], register=Technical.MFI):
...
class CMFCalculator(VolumeCalculator, variables=["cmf"], register=Technical.CMF):
...
class OBVCalculator(VolumeCalculator, variables=["obv"], register=Technical.OBV):
